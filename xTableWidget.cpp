...
void xTableWidget::slotUpdateTable(plist<pmap<pstring, pstring> > lmdata, pstring keyid)
{
    hlog(lmdata);
    ...
    else//更新,要根据keyid去找
    {
        ...
        for(int i=0;i<lmdata.size();i++)
        {
            ...
            //先找到索引
            int ifind=this->findIndexByKeyColumn(strNameKeyColumn.c_str(),keyid.c_str());
            //            hlog(ifind);
            //如果没找到的话,就增加一行
            if(ifind<0)
            {
...
//            hlog("##### < #####");
            ...
            for(int i=0;i<iCountReduce;i++)
            {
                ...
                {
                    pstring strKeyIdUI=this->getItemText(j,keyid.c_str()).toStdString();
