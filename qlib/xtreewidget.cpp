...
//    hlog(gallcsgl);
    ...
    {
        CSGLSystem systemi=gallcsgl.getSystemByIndex(i);
        QTreeWidgetItem* prooti=this->addRoot(systemi.strName.c_str());
        //再遍历每个站
        for(int j=0;j<systemi.size();j++)
        {
            //每个站名字是DTC+IP
            CSGLStation stationj=systemi.getStationByIndex(j);
//            pstring strNameStatation=stationj.strName+"("+stationj.strip+")";
            prooti->addChild(this->newJoint(stationj.strName.c_str()));
        }
    }
...
void xTreeWidget::itemClick(QTreeWidgetItem *item, int column)
{
    ...
    //获取本节点以及父节点的内容,来确定系统和站名字
    //此处不要转,转了就错了,控制台打印错误没事
    pstring strNameSystem=(parent->text(0).toStdString());
    pstring strNameStation=item->text(0).toStdString();
    hlog(strNameSystem,strNameStation);
//    sigTransSystemNameAndStationName(strNameSystem,strNameStation);
